...
Every segment is a single ES module file in `src/segments/`. This document is the authoritative specification for the segment API.

## Module Shape

A segment file MUST export exactly two things:
...
|-------|------|----------|-------------|
| `name` | `string` | yes | Unique segment identifier. Must match the filename (e.g., `model.js` exports `name: "model"`). Used in theme files to reference the segment. |
| `description` | `string` | yes | One-line human-readable description. Shown in `omc list`. |
| `requires` | `string[]` | no | External dependencies this segment needs. Example: `["git"]`. The framework checks availability at startup and skips segments whose deps are missing. Defaults to `[]`. |
| `defaultConfig` | `object` | no | Segment-specific configuration defaults. Merged under the theme's per-segment config and the user's per-segment overrides. Defaults to `{}`. |
...
The JSON object piped from Claude Code on stdin. The framework parses it and passes it to every segment's `render()`. Fields may be absent or null at any time -- segments must handle this.
...
Segment-specific configuration object. Resolved by the framework in this order (later wins):

1. `meta.defaultConfig` (from the segment itself)
2. `theme.segments["segment-name"]` (from the active theme file)
3. `userConfig.segments["segment-name"]` (from the user's config.json)

The segment receives the final merged result.

### Return Value

`render()` MUST return one of:

#### `{ text: string, style: string }` -- show the segment
...
#### `null` -- hide the segment

Return `null` when the segment has nothing to show. The framework removes it from the line and adjusts spacing. This is the correct way to conditionally hide a segment.

## Error Handling

If `render()` throws an exception, the framework catches it and treats the segment as if it returned `null`. The statusline never crashes because of a segment error. Errors are silently swallowed -- the statusline is a display layer, not a place for error reporting.

That said, segments should still handle errors internally. Relying on the framework catch is a last resort, not a design pattern.
...
`render()` should complete in under 50ms. The framework calls every segment on every statusline update, so slow segments degrade the entire experience.
...
## Checklist for New Segments

- [ ] File is in `src/segments/` and filename matches `meta.name`
